    ...
    task void processCommand(){
        if(! call Queue.empty()){
            ...
            case CMD_CLIENT_CLOSE:
                dbg(COMMAND_CHANNEL, "Command Type: ClientClose\n");
            ...
            case CMD_MSG:
                dbg(COMMAND_CHANNEL, "Command Type: Sending Msg\n");
                signal CommandHandler.msg(&buff[0]);
                break;

            case CMD_WHISPER:
                dbg(COMMAND_CHANNEL, "Command Type: Whispering\n");
                signal CommandHandler.whisper(&buff[0],&buff[1]);
                break;

            case CMD_LISTUR:
                dbg(COMMAND_CHANNEL, "Command Type: ListenUser\n");
                signal CommandHandler.listur();
                break;
